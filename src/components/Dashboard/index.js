...
const pieChartData = [
    ...
    { name: 'Shopify Store', value: 1175.78, color: '#40E0D0' },
  ];
    ...
    const renderLineChart = () => {
      ...
      const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
          const { orders, sales } = payload[0].payload;
          const avgOrderValue = (sales / orders).toFixed(2);
    
          return (
            <div className="custom-tooltip" style={{ backgroundColor: '#fff', padding: '10px', border: '1px solid #ccc' }}>
              <p className="label">{`Date: ${label}`}</p>
              <p>
                <span style={{ display: 'inline-block', width: '10px', height: '10px', backgroundColor: '#FFA500', borderRadius: '50%', marginRight: '5px' }}></span>
                {`Orders: ${orders}`}
              </p>
              <p>
                <span style={{ display: 'inline-block', width: '10px', height: '10px', backgroundColor: '#1da1f2', borderRadius: '50%', marginRight: '5px' }}></span>
                {`Sales: ${sales}`}
              </p>
              <p>   <span style={{ display: 'inline-block', width: '10px', height: '10px', backgroundColor: 'gray', borderRadius: '50%', marginRight: '5px' }}></span>
                 {`Avg Order Value: ${avgOrderValue}`}</p>
            </div>
          );
        }
      ...
      return(
        <ResponsiveContainer width="100%" height={500}>
          <LineChart data={lineChartData}
            ...
            <Line type="monotone" dataKey="sales" stroke="#1da1f2" activeDot={{ r: 8 }}/>
          </LineChart>
        </ResponsiveContainer>
      )
    }
        ...
        <div className="main-dashboard-card">
            ...
            <div className='center-card'>
                ...
                <div className='reacharts-card'>
                    <div className='line-chart-card'  style={{ height: window.innerWidth < 768 ? '50vh' : '90vh' }} >
                    ...
                    <div className='pie-chart-card' style={{ height: window.innerWidth < 768 ? '50vh' : '90vh' }} >
                      <div className='reacharts-heading-cart'>
